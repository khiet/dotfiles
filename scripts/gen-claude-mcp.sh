...
// Servers Claude already gets through another channel (e.g. the context-mode
// plugin ships its own MCP server); syncing them would create duplicates.
const SKIP = new Set(['context-mode']);
...
for (const [name, entry] of Object.entries(mcp)) {
  if (SKIP.has(name)) {
    console.log(`  - ${name} (skipped: provided by Claude plugin)`);
    continue;
  }
